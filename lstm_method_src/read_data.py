...
datapat = re.compile(r'.*(?P<datetime>[0-9]{4}[-][0-9]{2}[-][0-9]{2}.*[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}).*'
                    ...
                    +r'.*volume=(?P<volume>[0-9]*)'
                    +r'.*turnover=(?P<turnover>[0-9]*)'
...
def read_to_database(indexlist):
    for i in indexlist:
        if os.path.exists(__database_dir__ + str(i)) == False:
            ...
            with open(__database_dir__ + 'formatted_data_' + str(i), 'wb') as dbf:
                pickle.dump(res, dbf)
...
def read_data(indexlist):
    ...
    for i in indexlist:
        print(i)
        with open(__database_dir__ + 'formatted_data_' + str(i), 'rb') as dbf:
            tmp = pickle.load(dbf)
        for key in res:
